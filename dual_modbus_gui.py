...
class DualModbusGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Dual Modbus Reader - e2tango & PQI-DA")
    ...
    def read_pqi_registers(self):
        ...
        for e2t_addr, e2t_name, desc, pqi_addr, pqi_name, unit, conv in self.register_mapping:
            try:
                ...
                print(f"Error reading PQI register {pqi_name} at {pqi_addr}: {e}")
                data[pqi_name] = None
    ...
    def update_display(self, e2tango_data, pqi_data):
        ...
        for i, (e2t_addr, e2t_name, desc, pqi_addr, pqi_name, unit, conv) in enumerate(self.register_mapping):
            try:
                ...
                print(f"Error updating display for {e2t_name}: {e}")
    ...
    def update_power_monitoring(self, e2tango_data, pqi_data):
        ...
        for reg in energy_registers_e2t:
            if reg in e2tango_data and e2tango_data[reg] is not None:
                ...
                print(f"DEBUG: Updated e2tango {reg}: {e2tango_data[reg]} at {current_time}")
            else:
                print(f"DEBUG: e2tango {reg} not found or None in data: {reg in e2tango_data}")
        ...
        for reg in energy_registers_pqi:
            if reg in pqi_data and pqi_data[reg] is not None:
                ...
                print(f"DEBUG: Updated PQI {reg}: {pqi_data[reg]} at {current_time}")
            else:
                print(f"DEBUG: PQI {reg} not found or None in data: {reg in pqi_data}")
        ...
        for e2t_dev, e2t_reg, pqi_dev, pqi_reg in power_calculations:
            ...
            print(f"DEBUG: Power calc {e2t_reg}->{pqi_reg}: e2t={e2t_power}, pqi={pqi_power}")
    ...
    def update_energy_history(self, device, register, value, timestamp):
        ...
        old_count = len(self.energy_history[device][register])
        self.energy_history[device][register] = [
            (t, v) for t, v in self.energy_history[device][register] if t > cutoff_time
        ]
        new_count = len(self.energy_history[device][register])
        if old_count != new_count:
            print(f"DEBUG history: {device}.{register} cleaned {old_count}->{new_count} readings")
    ...
    def calculate_average_power(self, device, register, time_window, min_time=60.0):
        ...
        if (device not in self.energy_history or
            ...
            print(f"DEBUG calc: {device}.{register} - no history or < 2 readings")
            return None
        ...
        if len(readings_in_window) < 2:
            print(f"DEBUG calc: {device}.{register} - < 2 readings in window, total history: {len(history)}")
            return None
        ...
        print(f"DEBUG calc: {device}.{register} - time_diff: {time_diff:.1f}s, energy_diff: {energy_diff}, min_time: {min_time}")

        # Require minimum time span for reliable average
        if time_diff < min_time:
            print(f"DEBUG calc: {device}.{register} - time_diff {time_diff:.1f} < min_time {min_time}")
            return None

        # Convert Wh to W (energy/time), varh to var
        power_avg = (energy_diff / time_diff) * 3600.0  # Wh/s -> W

        print(f"DEBUG calc: {device}.{register} - SUCCESS: {power_avg:.3f} W")
        return power_avg
